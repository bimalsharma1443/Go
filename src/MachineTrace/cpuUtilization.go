...
func main() {
	...
	fmt.Println("Start to get cpu utilization")

	cpuStat, err := cpu.Info()

	if err != nil {
		panic("unable to get cpu information")
		return
	}
	...
	if err != nil {
		panic("unable to get cpu percentage")
		return
	}
	...
	for i, percentage := range percentages {
		fmt.Printf("Cpu utilization percentage for %d is %v\n", i, percentage)
	}

	fmt.Println(cpu.PercentWithContext(context.Background(), 0, true))
